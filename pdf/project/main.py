...
const metrics=j=>{const m=j.metrics||{},r=m.rag||{},g=m.answer_grounding||{},l=m.llm||{},d=m.document_extraction||{},flags=m.quality_flags||[];return ['Run metrics',`RAG: ${r.engine||'none'} | chunks ${r.chunks_total||0} | retrieved ${r.retrieved_count||0} | legal refs ${r.legal_reference_chunks||0}`,`Grounding: citations ${g.valid_citations||0}/${g.citations_total||0} valid | unsupported legal lines ${g.unsupported_legal_claim_lines||0}`,`LLM: ${l.provider||'none'} | ${l.response_mode||'none'} | attempted ${l.attempted?'yes':'no'} | fallback ${l.fallback_used?'yes':'no'}`,`Document: pages ${d.pages||0} | elements ${d.elements_total||0} | visual classified ${d.visual_elements_classified||0}`,`Flags: ${flags.length?flags.join(', '):'none'}`].join('\\n')};
const sources=j=>`${metrics(j)}\\n\\n${(j.retrieved||[]).map(x=>`${x.rank}. ${x.source_file||'reference'} ${x.page?`p.${x.page}`:''} ${x.kind||''}\\n${(x.text||'').slice(0,220)}`).join('\\n\\n')||'No sources returned.'}`;
