...
class VectorStore:
    ...
    def retrieve(self, query, top_k=TOP_K, pool_k=POOL_K):
        ...
        if self.faiss_index is not None and self.embedder is not None:
            ...
            ranked.append([int(i) for i in ids[0] if i >= 0])
        if self.tfidf_matrix is not None:
            qx = self.tfidf.transform([query])
            sim = cosine_similarity(qx, self.tfidf_matrix).ravel()
            ranked.append([int(i) for i in np.argsort(-sim)[:min(pool_k, len(self.chunks))]])
...
def chunks_from_records(records, case_id):
    ...
    for record in records:
        for i, text in enumerate(chunker.split(record.get("text", ""))):
            chunks.append({"chunk_id": f"{case_id}_C{len(chunks)+1:05d}", "text": text, "source_file": record.get("source_file"), "page": record.get("page"), "element_id": record.get("element_id"), "kind": record.get("kind", "document"), "chunk_index": i})
    return chunks
